...
/**
 * NTP (NyaSama Train Protocol) control handler.
 * Processes keyboard input and translates to train control commands.
 * Migrated from club.nsdn.nyasamarailway.event.NTPCtrlHandler / TrainController.doControl.
 *
 * This is platform-agnostic - keyboard binding is done in platform modules.
 */
public final class NtpControlHandler {
    ...
    public static boolean directionUp(MinecartData data) {
        ...
        int dir = data.getDir();
        if (dir == -1) { data.setDir(0); return true; }
        if (dir == 0) { data.setDir(1); return true; }
        return false;
    }
