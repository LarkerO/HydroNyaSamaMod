...
/**
 * NTP (NyaSama Train Protocol) motion calculation library. Contains all physics formulas for train
 * motion from the original TrainController. Migrated from
 * club.nsdn.nyasamarailway.util.TrainController (motion methods only).
 *
 * <p>All methods are static and platform-agnostic - they operate on primitive values rather than
 * Minecraft Entity objects.
 */
public final class NtpMotion {
  ...
  public static double calcVelocityBasic(
      double currentVelocity, int power, int brakeR, boolean highSpeed) {
  ...
  public static double calcVelocityWithAir(
      double currentVelocity, int power, int brakeR, double maxPower) {
    double velocity = Math.abs(currentVelocity);
    if (power > 0 && velocity < MIN_VELOCITY_THRESHOLD) {
      velocity = MIN_VELOCITY_THRESHOLD;
    }
    ...
    if (brakeR > 1) {
      double outP =
          maxPower / Math.pow(20.0, Math.E / 2.0) * Math.pow((double) power, Math.E / 2.0);
  ...
  public static double calcVelocityEuler(
      double currentVelocity, int power, int brakeR, double maxV) {
    double velocity = Math.abs(currentVelocity);
  ...
  public static double calcVelocitySlip(
      double currentVelocity, int power, int brakeR, double maxV) {
    double velocity = Math.abs(currentVelocity);
    double minV = MIN_VELOCITY_EULER;
    if (power > 0 && velocity < minV) velocity = minV;
  ...
  /** Calculate yaw from entity rotation. Original: 180.0 - cart.rotationYaw */
  ...
  /**
   * Deceleration model with electromagnetic braking. resistiveLoss = resistance * brakeRatio is a
   * constant braking force term (not velocity-proportional); this matches the original
   * Dynamics.LocoMotions formula.
   */
  ...
  private static double calcVelocityDownWithAir(
    ...
    double airDrag = 0.01 * velocity * velocity;
    return velocity
        - (friction * velocity + brakePower + resistiveLoss + airDrag) / mass * deltaTime;
  }

  private static double calcWithEuler(
      double velocity, double power, double brakeRelease, double maxVelocity, double deltaTime) {
